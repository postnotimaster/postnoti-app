...
export const DeliveryModal = ({
        ...
        try {
            ...
            try {
                if (guideStr.startsWith('{')) {
                    ...
                    setGuidelines(parsed.text || '?∞Ìé∏Î¨??ÑÎã¨ ?†Ï≤≠???òÏãúÎ©?ÏßÄ?ïÎêú Ï£ºÏÜåÎ°?Î∞∞ÏÜ°???úÎ¶Ω?àÎã§.');
                } else {
                    setGuidelines(guideStr || '?∞Ìé∏Î¨??ÑÎã¨ ?†Ï≤≠???òÏãúÎ©?ÏßÄ?ïÎêú Ï£ºÏÜåÎ°?Î∞∞ÏÜ°???úÎ¶Ω?àÎã§.');
                    setPaymentInfo(null);
                }
            } catch (e) {
                setGuidelines(guideStr || '?∞Ìé∏Î¨??ÑÎã¨ ?†Ï≤≠???òÏãúÎ©?ÏßÄ?ïÎêú Ï£ºÏÜåÎ°?Î∞∞ÏÜ°???úÎ¶Ω?àÎã§.');
    ...
    // ???òÍ≤Ω Î©îÏãúÏßÄ ?òÏã† (Daum Ï£ºÏÜå Í≤Ä??
    useEffect(() => {
    ...
    const handleSubmit = async () => {
        ...
        if (missing.length > 0) {
            ...
            setErrorText('Î∂âÏ??âÏúºÎ°??úÏãú???ÑÏàò ??™©??Î™®Îëê ?ÖÎ†•??Ï£ºÏÑ∏??');
            return;
        }
        ...
        try {
            ...
            // 1. ?ÑÎ°ú??Ï£ºÏÜå ?ÖÎç∞?¥Ìä∏ (Ï≤¥ÌÅ¨??Í≤ΩÏö∞)
            if (saveAsDefault) {
            ...
            setErrorText(e.message || '?†Ï≤≠???§Ìå®?àÏäµ?àÎã§.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const getStatusInfo = (status: MailDeliveryStatus) => {
        switch (status) {
            case 'pending': return { label: '?ëÏàò?ÄÍ∏?, color: '#D97706', bg: '#FEF3C7', icon: 'time-outline' };
            case 'received': return { label: '?ÖÍ∏à?ÄÍ∏?, color: '#2563EB', bg: '#DBEAFE', icon: 'wallet-outline' };
            case 'paid': return { label: 'Î∞úÏÜ°Ï§ÄÎπ?, color: '#4338CA', bg: '#E0E7FF', icon: 'cube-outline' };
            case 'shipped': return { label: 'Î∞úÏÜ°?ÑÎ£å', color: '#059669', bg: '#D1FAE5', icon: 'checkmark-circle-outline' };
            ...
            <Modal visible={visible} animationType="slide" onRequestClose={() => setStep('form')}>
                <SafeAreaView style={styles.postcodeSafe}>
                    <View style={styles.postcodeHeader}>
                        ...
                        <Text style={styles.postcodeTitle}>Ï£ºÏÜå Í≤Ä??/Text>
                    </View>
                    <View style={{ flex: 1 }}>
                        {Platform.OS === 'web' ? (
                            <iframe src={postcodeUrl} style={{ border: 'none', width: '100%', height: '100%' }} title="Ï£ºÏÜå Í≤Ä?? />
                        ) : (
                            <WebView
            ...
            >
                <View style={styles.overlay}>
                    <View style={[styles.content, { alignItems: 'center', paddingVertical: 40 }]}>
                        ...
                        <Text style={[styles.title, { marginTop: 20, marginBottom: 10 }]}>?†Ï≤≠ ?ÑÎ£å!</Text>
                        <Text style={{ textAlign: 'center', color: '#64748B', lineHeight: 22, marginBottom: 30 }}>
                            ?∞Ìé∏Î¨??ÑÎã¨ ?†Ï≤≠??"\n"}?ïÏÉÅ?ÅÏúºÎ°??ëÏàò?òÏóà?µÎãà??
                        </Text>
                        <PrimaryButton
                            label="?¥Ïó≠ ?ïÏù∏?òÍ∏∞"
                    ...
                    <View style={styles.header}>
                        <View style={styles.tabs}>
                            <Pressable onPress={() => setActiveTab('request')} style={[styles.tab, activeTab === 'request' && styles.activeTab]}>
                                <Text style={[styles.tabText, activeTab === 'request' && styles.activeTabText]}>?†Ï≤≠?òÍ∏∞</Text>
                            </Pressable>
                            <Pressable onPress={() => setActiveTab('list')} style={[styles.tab, activeTab === 'list' && styles.activeTab]}>
                                <Text style={[styles.tabText, activeTab === 'list' && styles.activeTabText]}>?†Ï≤≠?ÑÌô©</Text>
                            </Pressable>
                        </View>
                        ...
                        <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
                            ...
                            <View style={styles.form}>
                                <View style={styles.inputGroup}>
                                    <Text style={[styles.label, missingFields.includes('name') && { color: '#EF4444' }]}>?òÎ†π???ïÎ≥¥ *</Text>
                                    <TextInput style={[styles.input, missingFields.includes('name') && styles.errorInput]} value={name} onChangeText={(t) => { setName(t); setMissingFields(prev => prev.filter(f => f !== 'name')); }} placeholder="?±Ìï®" />
                                    <TextInput style={[styles.input, { marginTop: 8 }, missingFields.includes('phone') && styles.errorInput]} value={phone} onChangeText={(t) => { setPhone(t); setMissingFields(prev => prev.filter(f => f !== 'phone')); }} placeholder="?∞ÎùΩÏ≤? keyboardType="phone-pad" />
                                </View>

                                <View style={styles.inputGroup}>
                                    <View style={styles.labelRow}>
                                        <Text style={[styles.label, missingFields.includes('address') && { color: '#EF4444' }]}>Î∞∞ÏÜ° Ï£ºÏÜå *</Text>
                                        <Pressable onPress={() => setStep('postcode')} style={styles.searchBtn}>
                                            <Text style={styles.searchBtnText}>Ï£ºÏÜå Ï∞æÍ∏∞</Text>
                                        </Pressable>
                                    </View>
                                    ...
                                    <View style={styles.quickInputRow}>
                                        {savedAddress && (
                                            <Pressable style={styles.premiumQuickBtn} onPress={() => handleSelectHistory(savedAddress)}>
                                                ...
                                                <Text style={styles.premiumQuickBtnText}>?±Î°ù Ï£ºÏÜå ?¨Ïö©</Text>
                                            </Pressable>
                                        )}
                                        {recentAddresses.length > 0 && (
                                            <Pressable style={styles.useLastAddressBtn} onPress={() => handleSelectHistory(recentAddresses[0])}>
                                                ...
                                                <Text style={styles.useLastAddressText}>ÏµúÍ∑º Î∞∞ÏÜ°ÏßÄ</Text>
                                            </Pressable>
                                        )}
                                    </View>
                                    ...
                                    <TextInput
                                        ...
                                        placeholder="Ï£ºÏÜå (ÏßÅÏ†ë ?ÖÎ†• ?êÎäî Í≤Ä??"
                                        editable={Platform.OS === 'web' || !address}
                                    />
                                    <TextInput style={[styles.input, { marginTop: 8 }]} value={addressDetail} onChangeText={setAddressDetail} placeholder="?ÅÏÑ∏ Ï£ºÏÜå" />
                                    ...
                                    <View style={styles.saveDefaultRow}>
                                        ...
                                        <Text style={styles.saveDefaultText}>?ÖÎ†•??Ï£ºÏÜåÎ•???Í∏∞Î≥∏ Ï£ºÏÜåÎ°??Ä?•ÌïòÍ∏?/Text>
                                    </View>
                                </View>
                            </View>
                            <View style={{ height: 30 }} />
                            <PrimaryButton label="?ÑÎã¨ ?†Ï≤≠?òÍ∏∞" onPress={handleSubmit} loading={loading} disabled={loading} />
                        ...
                        <FlatList
                                    ...
                                    <View style={styles.requestCard}>
                                        ...
                                        {item.status === 'received' && (
                                            <View style={styles.infoNote}>
                                                {paymentInfo ? (
                                                    <View>
                                                        <Text style={styles.infoNoteText}>Í∏àÏï°: {paymentInfo.amount}</Text>
                                                        <Text style={styles.infoNoteText}>?ÖÍ∏àÍ≥ÑÏ¢å: {paymentInfo.bank} {paymentInfo.account}</Text>
                                                        <Text style={styles.infoNoteText}>?àÍ∏àÏ£? {paymentInfo.holder}</Text>
                                                    </View>
                                                ) : (
                                                    <Text style={styles.infoNoteText}>Í≤∞Ï†ú ?ïÎ≥¥Í∞Ä ?ÑÏßÅ ?§Ï†ï?òÏ? ?äÏïò?µÎãà??</Text>
                                                )}
                                            </View>
                                        )}
                                    </View>
                            ...
                            ListEmptyComponent={
                                <View style={styles.emptyBox}>
                                    ...
                                    <Text style={styles.emptyText}>?†Ï≤≠ ?¥Ïó≠???ÜÏäµ?àÎã§.</Text>
                                </View>
                            }
                        />
                    )}
