...
export const TenantManagement = forwardRef(({ companyId, onComplete, onCancel }: TenantManagementProps, ref) => {
    ...
    // BackHandler Î°úÏßÅ?Ä Parent Ïª¥Ìè¨?åÌä∏?êÏÑú useImperativeHandle(ref)Î•??µÌï¥
    // handleBack ?®ÏàòÎ•?ÏßÅÏ†ë ?∏Ï∂ú?òÎèÑÎ°??¥Í???(Î¶¨Ïä§??Ï§ëÎ≥µ/Ï∂©Îèå Î∞©Ï?)
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // ?ÅÌÉúÍ∞íÏóê ?∞Î•∏ is_active Í∞ïÏ†ú ?ôÍ∏∞??(?òÏúÑ ?∏Ìôò???†Ï?)
            const finalTenant = { ...editingTenant };
            if (!finalTenant.status) {
                finalTenant.status = finalTenant.is_active ? '?ÖÏ£º' : '?¥Í±∞';
            }
            finalTenant.is_active = finalTenant.status === '?ÖÏ£º';
    ...
    const handleShareInvite = async (tenant: Tenant) => {
        ...
        const message = `[?¨Ïä§?∏ÎÖ∏??Í≥µÏú†?§Ìîº??\n${tenant.name}?? ?ÑÏö© ?∞Ìé∏?®Ïù¥ Ï§ÄÎπÑÎêò?àÏäµ?àÎã§!\n\n?ÑÎûò ÎßÅÌÅ¨Î•??åÎü¨ Î°úÍ∑∏???ÜÏù¥ Î∞îÎ°ú ?∞Ìé∏Î¨ºÏùÑ ?ïÏù∏?òÍ≥†, ?åÎ¶º ?±ÏùÑ ?§Ïπò??Î≥¥ÏÑ∏??\n\n?ëâ ?ÑÏö© ?∞Ìé∏???¥Í∏∞:\n${link}`;
        
        Alert.alert('?ÖÏ£º??Ï¥àÎ?', '?¥Îñ§ Î∞©Ïãù?ºÎ°ú ?ÑÎã¨?òÏãúÍ≤†Ïäµ?àÍπå?', [
            {
                text: 'Î¨∏Ïûê(SMS)Î°?Î≥¥ÎÇ¥Í∏?,
                onPress: async () => {
                    ...
                    try {
                        ...
                        Alert.alert('?§Î•ò', 'Î¨∏Ïûê ?±ÏùÑ ?????ÜÏäµ?àÎã§.');
                    }
                }
            },
            {
                text: 'ÎßÅÌÅ¨Îß?Î≥µÏÇ¨?òÍ∏∞',
                onPress: async () => {
                    await Clipboard.setStringAsync(message);
                    Alert.alert('Î≥µÏÇ¨ ?ÑÎ£å', 'Ï¥àÎ? Î©îÏãúÏßÄ?Ä ?ÑÏö© ÎßÅÌÅ¨Í∞Ä ?¥Î¶ΩÎ≥¥Îìú??Î≥µÏÇ¨?òÏóà?µÎãà?? Ïπ¥Ïπ¥?§ÌÜ° ?±Ïóê Î∂ôÏó¨?£Í∏∞ ?òÏÑ∏??');
                }
            },
        ...
        return (
            ...
            >
                ...
                <ScrollView contentContainerStyle={styles.editForm}>
                    ...
                    <View style={styles.compactInputGroup}>
                        <Text style={styles.compactLabel}>?∏Ïã§</Text>
                        <TextInput
                            style={styles.compactInput}
                    ...
                    <View style={styles.compactInputGroup}>
                        <Text style={styles.compactLabel}>?ÑÌôîÎ≤àÌò∏</Text>
                        <TextInput
                            style={styles.compactInput}
                    ...
                    <View style={{ marginTop: 8, marginBottom: 10 }}>
                        <Text style={[styles.compactLabel, { marginBottom: 6, width: '100%' }]}>?ìã ?ÖÏ£º ?ÅÌÉú</Text>
                        <View style={{ flexDirection: 'row', gap: 6 }}>
                            {['?ÖÏ£º', '?¥Í±∞', '?êÏóÖ', '?¥Ï†Ñ', '?åÏû¨Î∂àÎ™Ö'].map((st) => {
                                const currentStatus = editingTenant.status || (editingTenant.is_active ? '?ÖÏ£º' : '?¥Í±∞');
                                ...
                                return (
                                    <Pressable
                                        ...
                                        style={{
                                            ...
                                            borderWidth: 1,
                                            alignItems: 'center',
                                            backgroundColor: isSelected ? (st === '?ÖÏ£º' ? '#10B981' : '#F1F5F9') : '#fff',
                                            borderColor: isSelected ? (st === '?ÖÏ£º' ? '#10B981' : '#CBD5E1') : '#E2E8F0'
                                        }}
                                    >
                                        <Text style={{
                                            fontSize: 11,
                                            fontWeight: '800',
                                            color: isSelected ? (st === '?ÖÏ£º' ? '#fff' : '#475569') : '#94A3B8'
                                        }}>
                                            {st}
                                        </Text>
                                    </Pressable>
                    ...
                    <View style={[styles.compactInputGroup, { justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 0 }]}>
                        <View>
                            <Text style={styles.compactLabel}>{"?ÑÎ¶¨ÎØ∏ÏóÑ ?úÎπÑ??}</Text>
                        </View>
                        <Switch
                    ...
                    <View style={{ marginTop: 8, marginBottom: 10 }}>
                        <Text style={[styles.compactLabel, { marginBottom: 6, width: '100%' }]}>?ñºÔ∏??¨ÏßÑ ?êÎèô ??†ú</Text>
                        <View style={{ flexDirection: 'row', gap: 6 }}>
                            {[
                                { label: '1Ï£?, days: 7 },
                                { label: '2Ï£?, days: 14 },
                    ...
                    <View style={{
                        ...
                        >
                            <Text style={{ color: '#64748B', fontWeight: '800', fontSize: 15 }}>Ï∑®ÏÜå</Text>
                        </Pressable>
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Pressable onPress={() => { setEditingTenant({ company_id: companyId, is_active: true, is_premium: false, status: '?ÖÏ£º', retention_days: 14 }); setIsEditing(true); }} style={styles.addBtn}>
                            ...
                            <Pressable key={t.id} style={styles.tenantCard} onPress={() => { setEditingTenant(t); setIsEditing(true); }}>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        ...
                                        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                            ...
                                            <Text style={{ fontSize: 11, color: '#64748B', fontWeight: '600' }}>
                                                Î≥¥Í? <Text style={{ color: '#4F46E5' }}>{t.retention_days === 0 ? '?ÅÍµ¨' : `${(t.retention_days || 14) / 7}Ï£?}</Text>
                                            </Text>
                                            <Text style={{ fontSize: 10, color: '#CBD5E1' }}>|</Text>
                                            <Text style={{ fontSize: 11, color: '#64748B', fontWeight: '600' }}>
                                                Í∞úÎ¥â <Text style={{ color: '#059669' }}>{total > 0 ? `${read}/${total}` : '-'}</Text>
                                            </Text>
                                        </View>
                                    </View>
                                </View>
                                <View style={styles.cardActions}>
                                    <Pressable onPress={() => handleShareInvite(t)} style={[styles.editBtn, { marginBottom: 12 }]}>
                                        <Text style={[styles.editBtnText, { color: '#059669' }]}>{'?ì© Ï¥àÎ?'}</Text>
                                    </Pressable>
