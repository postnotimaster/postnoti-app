...
async function check() {
    const { data } = await supabase.from('mail_logs').select('*').limit(1);
    if (data && data.length > 0) {
        console.log('Columns in mail_logs:', Object.keys(data[0]).join(', '));
    } else {
        console.log('mail_logs table is empty, checking structure via query...');
        // Try to get headers
        const { data: cols } = await supabase.from('mail_logs').select('*').limit(0);
